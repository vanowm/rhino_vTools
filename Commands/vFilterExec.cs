...
public sealed class vFilterExec : Command
{
  ...
  private const string SettingsSection = "vFilterExec";
  ...
  protected override Result RunCommand(RhinoDoc doc, RunMode mode)
  {
    ...
    if (!TryGetCommand(settings.Command, out var command, out var commandResult))
      return commandResult;

    if (!TryGetFilter(settings.Filter, out var filter, out var filterResult))
      return filterResult;

    SaveSettings(command, filter.CanonicalSpec);
  ...
  private static LauncherSettings LoadSettings()
  {
    return ToolsOptionStore.Read(
      ...
      section =>
      {
        ...
        var filter = "Curves";
        if (ToolsOptionStore.TryGetString(section, CommandKey, out var savedCommand))
          command = savedCommand;
        if (ToolsOptionStore.TryGetString(section, FilterKey, out var savedFilter))
          filter = savedFilter;
        return new LauncherSettings(command, filter);
      });
  }

  private static void SaveSettings(string command, string filter)
  {
    if (ToolsOptionStore.Update(
          SettingsSection,
          section =>
          {
            section[CommandKey] = command;
            section[FilterKey] = filter;
          }))
    {
      return;
    }

    Log.Write(Tag, $"could not save settings: {ToolsOptionStore.LastError}");
  }
    ...
    out Result commandResult)
  {
    ...
    getter.AcceptNothing(!string.IsNullOrWhiteSpace(currentCommand));
    getter.EnableTransparentCommands(true);
    if (!string.IsNullOrWhiteSpace(currentCommand))
      getter.SetDefaultString(currentCommand);
    ...
    command = result == GetResult.Nothing
      ? currentCommand.Trim()
    ...
    out Result commandResult)
  {
    ...
    getter.EnableTransparentCommands(true);
    getter.SetDefaultString(string.IsNullOrWhiteSpace(currentFilter) ? "Curves" : currentFilter);
    ...
    {
      GetResult.Nothing => currentFilter,
  ...
  private sealed record LauncherSettings(string Command, string Filter);
