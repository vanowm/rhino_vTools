...
public sealed class vFacing : Command
{
  ...
  private static List<double> FindCornerParams(Curve curve, double minAngleDeg)
  {
    ...
    var count  = poly.IsClosed ? n : n - 1; // closed: also check last→first junction
    ...
    for (var i = 0; i < count; i++)
    {
      ...
      if (i < n - 1)
      {
        // Interior junction: add the parameter at end of segment i
        var dom = poly.SegmentDomain(i);
        result.Add(dom.Max);
      }
      // For closed-curve wrap-around (i == n-1): junction is at the seam, no param needed
    }
